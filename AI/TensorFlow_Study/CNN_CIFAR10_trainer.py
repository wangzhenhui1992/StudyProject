...
x = tf.placeholder(tf.float32, shape=[None, 3, 32, 32], name='x')  # 定义输入参数x  784=28*28
y_ = tf.placeholder(tf.int64, shape=[None, 10], name='y_')  # 定义输入参数y_

x_image = tf.transpose(x, perm=[0, 2, 3, 1])  # 将x转成28*28的格式
# Layer1  第一卷积层 要获得32个特征图
W_conv1 = weight_variable([5, 5, 3, 32 * 3])  # 定义第一个卷积层的w,卷积核大小是5*5,输入数据的特征图数量为1，该层输出数据的特征图数量为32，所以是5,5,1,32
b_conv1 = bias_variable([32 * 3])  # 定义第一个卷积层的b,因为一个特征图内偏置共享（参考CNN的理论说明），所以b的个数与输出数据的特征图数量相同32
...
W_conv2 = weight_variable(
    [5, 5, 32 * 3, 64 * 3])  # 定义第二个卷积层的w,卷积核大小是5*5,输入数据（上一层的输出数据）的特征图数量为1，该层输出数据的特征图数量为32，所以是5,5,32,64
b_conv2 = bias_variable([64 * 3])  # 定义第二个卷积层的b,因为一个特征图内偏置共享（参考CNN的理论说明），所以b的个数与输出数据的特征图数量相同64
...
h_pool2_flat = tf.reshape(h_pool2, [-1, 8 * 8 * 64 * 3])  # 把结果转成一个数组形式的，相当于有3133=7*7*64个特征值

# Layer5 第一全连接层 要获得1024个特征《值》，是值...是值...(重要的事情说三遍)
W_fc1 = weight_variable([8 * 8 * 64 * 3, 1024])  # 前一层的特征数3133=7*7*64,想要获得1024个特征，因为是全连结，所以是[7 * 7 * 64, 1024]
...
cross_entropy = tf.reduce_mean(
    tf.nn.softmax_cross_entropy_with_logits(labels=y_, logits=y_conv))  # 定义交叉熵的计算方式 这里是softmax交叉熵
...
data_set.init_reader(r'./data')
with tf.Session() as sess:
    sess.run(tf.global_variables_initializer())  # 全局初始化变量
    for i in range(3000):  # 定义训练次数2000
        x_batch, y_batch = data_set.get_batch_data(50)  # 获取下50条数据作为batch训练数据，变相定义batch大小50
        if i % 100 == 0:
            train_accuracy = accuracy.eval(feed_dict={
                x: x_batch, y_: y_batch, keep_prob: 1.0})
            print('step %d, training accuracy %g' % (i, train_accuracy))  # 每一百步，输出准确率
        train_step.run(feed_dict={x: x_batch, y_: y_batch, keep_prob: 0.5})  # 使用batch训练数据进行训练，保留率50%
    test_data_x, test_data_y = data_set.get_test_data()
    print('test accuracy %g' % accuracy.eval(feed_dict={
        x: test_data_x, y_: test_data_y, keep_prob: 1.0}))  # 输出测试数据，准确率
